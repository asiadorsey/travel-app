...
const SignUpForm: React.FC<SignUpFormProps> = ({ onSuccess, onSwitchToSignIn }) => {
    const { signUpWithEmail, firebaseError } = useFirebase(); // Get the signup function and error from context
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            if (user) {
                onSuccess(); // Call success callback if user is created
            } else {
                // If user is null, an error was already set by useFirebase
                setError(firebaseError || "Sign up failed. Please try again.");
            }
        } catch (e: any) {
